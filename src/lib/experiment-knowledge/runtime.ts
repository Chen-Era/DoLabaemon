...
import { listRuntimeExperimentKnowledgeEntries } from "@/lib/knowledge/runtime-store";
import { scoreExperimentKnowledgeEntry } from "@/lib/experiment-knowledge/scoring";
import type { ExperimentKnowledgeCatalog, ExperimentKnowledgeMatch, ExperimentKnowledgeRetrievalResult } from "@/lib/experiment-knowledge/types";

const staticCatalog = rawCatalog as ExperimentKnowledgeCatalog;

function uniq<T>(items: T[]) {
  return [...new Set(items)];
}

export async function retrieveExperimentKnowledgeRuntime(input: {
  customExperimentName: string;
  experimentContext?: string | null;
  directionCode?: string | null;
}): Promise<ExperimentKnowledgeRetrievalResult> {
  const searchText = [input.customExperimentName, input.experimentContext].filter(Boolean).join(" | ");
  const runtimeEntries = await listRuntimeExperimentKnowledgeEntries();
  const catalog = runtimeEntries.length ? runtimeEntries : staticCatalog;

  const matchedEntries: ExperimentKnowledgeMatch[] = catalog
    .map((entry) => {
  ...
  const requiredTemplateHints = uniq(topMatches.flatMap((item) => item.entry.requiredReagentTemplates.map((template) => template.nameZh)));
  const recommendedTemplateHints = uniq(topMatches.flatMap((item) => item.entry.recommendedReagentTemplates.map((template) => template.nameZh)));
