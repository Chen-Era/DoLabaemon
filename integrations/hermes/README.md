...
把 [Hermes Agent](https://github.com/NousResearch/hermes-agent)（NousResearch 开源的自改进 agent）当作本系统的**异步知识管家**：它在服务器上按定时任务研究试剂、产出结构化的试剂知识 JSONL；项目侧脚本校验后导入本地试剂知识库（`ReagentKnowledgeEntry` 表）。知识库越厚，主流程检索置信度越高，联网验证就越少被触发，识别响应也越快。
...
3. **质量**：研究型产出需要多轮检索核对，适合离线慢慢做；产出后还要过项目侧的严格校验（枚举、正则、置信度），不合格行级拒收——这道闸门在同步路径上没有位置。
