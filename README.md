...
<p align="center">
  科研实验室的试剂库存管理与实验可行性判断系统<br />
  Reagent inventory management and experiment-readiness checks for research laboratories
</p>
...
在线体验 / Live demo: [dorlabaemon.era.ac.cn](https://dorlabaemon.era.ac.cn)

## 项目要解决什么问题 / What this project solves
...
```mermaid
flowchart LR
  A["录入试剂<br/>Add reagents"] --> B["确认字段与分类<br/>Confirm fields and categories"]
  B --> C["检查实验条件<br/>Check experiment readiness"]
  C --> D["在实验室内协作<br/>Collaborate in the laboratory"]
```

| 实验准备中的问题 / Preparation issue | Dorlabaemon 的做法 / Dorlabaemon approach |
| --- | --- |
| 试剂记录只有名称或货号，难以按实验用途核对。<br /><br />Reagent records contain only a name or catalog number, making it hard to check their experimental use. | 将试剂整理为结构化字段，并用抗体、引物、内参、细胞培养、转染和筛选等标签描述用途。<br /><br />Organize reagents into structured fields and describe their use with tags for antibodies, primers, reference controls, cell culture, transfection, and selection. |
| 知道库存里有什么，却不清楚能否开始一项实验。<br /><br />The inventory is known, but it is unclear whether an experiment can begin. | 将库存映射到实验的必需项和推荐项，列出已满足、待补充和需要注意的条件。<br /><br />Map inventory to required and recommended experiment items, then list fulfilled, missing, and notable conditions. |
| 团队成员各自维护记录，实验室之间又需要边界。<br /><br />Team members maintain separate records while laboratories still need clear boundaries. | 以实验室为单位共享库存；PI 和管理员可邀请成员，实验室数据相互隔离。<br /><br />Share inventory within a laboratory. PIs and administrators can invite members, while laboratory data stays isolated. |

## 项目亮点 / Highlights

### 从库存记录走到实验条件 / From inventory records to experiment readiness
...
### AI 负责整理，人负责确认 / AI organizes, people confirm
...
### 规则与知识可以维护 / Maintainable rules and knowledge
...
### 一套工作区服务团队协作 / One workspace for team collaboration
...
| --- | --- |
| 试剂库存<br />Reagent inventory | 新建、编辑和删除试剂记录；按名称、货号、标签或靶点筛选；支持库存增减、批量删除和复制导出。<br /><br />Create, edit, and delete reagent records. Filter by name, catalog number, tag, or target. Adjust quantities, delete in batches, and copy or export selected records. |
| 实验可行性判断<br />Experiment-readiness checks | 根据库存检查实验的必需项与推荐项，并说明缺失的材料。<br /><br />Check required and recommended items against inventory and identify missing materials. |
| 知识维护<br />Knowledge maintenance | 可配置 Skill 与 MCP 服务；AI 解析会保留自检记录，并可按实验室策略启用知识自动写回。<br /><br />Configure Skills and MCP services. AI parsing keeps self-check records, and knowledge write-back can be enabled by laboratory policy. |
| 多实验室协作<br />Multi-laboratory collaboration | 实验室内成员共享库存；实验室之间的数据相互隔离。PI 和管理员可以邀请成员。<br /><br />Members share inventory within a laboratory, while data is isolated between laboratories. PIs and administrators can invite members. |

## 快速开始 / Quick start

### DEMO 模式，约 30 秒 / DEMO mode, about 30 seconds
...
npm install
cp .env.example .env
# 将 .env 中的 DEMO_MODE 改为 "true" / Set DEMO_MODE="true" in .env
npm run dev
```

打开 / Open [http://localhost:3000](http://localhost:3000)。

### 数据库模式，macOS / Database mode, macOS
...
### 手动初始化数据库 / Initialize the database manually
...
### 推荐配置：MiMo Token Plan / Recommended configuration: MiMo Token Plan
...
| 配置 / Setting | 用途 / Purpose |
| --- | --- |
| `DEMO_MODE` | `true` 使用演示数据，`false` 使用 PostgreSQL。<br /><br />Use demo data with `true`; use PostgreSQL with `false`. |
| `DATABASE_URL` | PostgreSQL 连接字符串。<br /><br />PostgreSQL connection string. |
| `NEXTAUTH_URL`、`NEXTAUTH_SECRET` | 登录地址与会话加密密钥。本地开发地址为 `http://localhost:3000`。<br /><br />The sign-in URL and session encryption secret. The local development URL is `http://localhost:3000`. |
| `OPENAI_BASE_URL`、`OPENAI_API_KEY`、`OPENAI_MODEL`、`OPENAI_VISION_MODEL` | OpenAI 兼容模型服务的地址、密钥与模型名称。<br /><br />Endpoint, key, and model names for an OpenAI-compatible model service. |
| `REAGENT_SEARCH_*` | 联网核验所需的搜索服务配置。<br /><br />Search-service settings for online verification. |
| `LLM_ENABLED_SKILLS`、`LLM_ENABLED_MCP_SERVERS` | 启用的 Skill 与 MCP 服务。<br /><br />Enabled Skills and MCP services. |
| `LLM_SELF_CHECK_ENABLED`、`LLM_AUTO_LEARN_ENABLED` | 是否启用 AI 自检与自动学习写回。<br /><br />Whether to enable AI self-checks and automatic learning write-back. |
| `LLM_REASONING_EFFORT` | 模型推理等级，可选 `off`、`low`、`medium` 或 `high`。<br /><br />Model reasoning level: `off`, `low`, `medium`, or `high`. |
| `LLM_KNOWLEDGE_VERIFY_*` | 知识库高置信命中时跳过联网验证的开关与阈值。<br /><br />The switch and threshold for skipping online verification on high-confidence knowledge-base matches. |
| `HERMES_KNOWLEDGE_EXPORT_PATH` | Hermes 导出 JSONL 的路径。<br /><br />Path to Hermes-exported JSONL. |
...
- Next.js 16、React 19、TypeScript 5 与 Tailwind CSS 4<br />Next.js 16, React 19, TypeScript 5, and Tailwind CSS 4
- Electron 40
- Prisma 6 与 PostgreSQL<br />Prisma 6 and PostgreSQL
...
| --- | --- |
| [系统架构 / Architecture](docs/architecture.md) | 前端、API、数据库、鉴权、AI 编排和知识库模块。<br /><br />Frontend, API, database, authentication, AI orchestration, and knowledge-base modules. |
| [API 文档 / API](docs/api.md) | 路由与请求用途。<br /><br />Routes and their request purposes. |
| [规则设计 / Rule design](docs/rule-design.md) | 实验必需项、推荐项与方向规则。<br /><br />Required items, recommended items, and research-direction rules. |
| [桌面客户端 / Desktop client](docs/desktop-client.md) | Electron 客户端的构建、配置与安全边界。<br /><br />Build, configuration, and security boundaries for the Electron client. |
| [服务器部署 / Server deployment](docs/deployment-server.md) | Docker、Caddy 和 Nginx 的生产部署。<br /><br />Production deployment with Docker, Caddy, and Nginx. |
| [Hermes 知识管家 / Hermes knowledge steward](integrations/hermes/README.md) | Hermes 的部署、定时产出与导入流程。<br /><br />Hermes setup, scheduled output, and import workflow. |
| [Hermes 集成机制 / Hermes integration](docs/hermes-integration.md) | 项目侧的校验、写库和检索置信度机制。<br /><br />Project-side validation, persistence, and retrieval-confidence behavior. |
